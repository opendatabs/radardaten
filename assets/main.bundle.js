...
var AdminComponent = /** @class */ (function () {
    ...
    //   }
    // }
    AdminComponent.prototype.onUserRowSelect = function (event) {
        var _this = this;
        if (event.selected.length) {
            this.radarService.updateRecordCount(event.selected[0]).subscribe(
            // res => console.log('Recordcount updated', res),
            function (err) { return console.error('Reocrdcount could not be updated', err); });
            this.radarService.getRadar(event.selected[0].id).subscribe(function (e) { return _this.source.update(event.selected[0], e); } // update(selected object, new values)
            );
        }
    };
...
var AddRecordsBtnComponent = /** @class */ (function () {
    function AddRecordsBtnComponent(modalService, radarService, sailsClientService) {
    ...
    AddRecordsBtnComponent.prototype.ngOnInit = function () {
        var _this = this;
        this.sub = this.sailsClientService.on('newRecords').subscribe(function () {
            if (_this.foundMatches > 0) {
                _this.creationCounter += 50;
                _this.progress = Math.ceil((_this.creationCounter / _this.foundMatches) * 100);
            }
    ...
    AddRecordsBtnComponent.prototype.updateRecordCount = function () {
        this.radarService.updateRecordCount(this.rowData).subscribe(function (res) { return console.log('Recordcount updated', res); }, function (err) { return console.error('Reocrdcount could not be updated', err); });
    };
    __decorate([
        core_1.Input(),
